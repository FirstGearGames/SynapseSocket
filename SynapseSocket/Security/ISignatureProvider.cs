...
/// Implementations should derive a stable identifier for a remote peer from
/// properties such as its <see cref="IPEndPoint"/>, device info, or handshake
/// payload. Signatures are used for identity, blacklisting, and spoofing
/// mitigation.
/// </summary>
public interface ISignatureProvider
{
    ...
    /// The payload contains a unique nonce per handshake. Implementations MUST incorporate
    /// <paramref name="handshakePayload"/> into the computed signature when it is non-empty;
    /// implementations that hash only the endpoint bypass the engine's handshake replay cache
