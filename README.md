...
8. Releases the pooled buffers held by connections closed since the previous poll, including any closed by `Disconnect` or `Connect` between polls.
...
| `ConnectionClosed` | A connection ends: timeout, kick, `Disconnect` (on both sides), or being replaced by a new `Connect` to the same endpoint. On the other side of such a reconnect, the fresh handshake raises `ConnectionClosed` and then at once `ConnectionEstablished` for the same connection object, which stays live rather than being torn down |
| `ConnectionReleased` | A closed connection's pooled buffers have been released, which is the point to drop any reference kept past `ConnectionClosed`. Raised once per connection: at the end of the `Poll` that closed it, straight after `ConnectionClosed` on the replaced side of a reconnect (the object then goes on as the new session), and for every open connection on `Stop` or `Dispose`, which raise no `ConnectionClosed` |
...
- **Sending on a closed `SynapseConnection` throws.** A closed connection is dead for good: connection objects are never reused, so a reference kept past close can never reach another session. `Send` on one throws `InvalidOperationException` and `Disconnect` on one does nothing. Its pooled buffers are released at the end of the `Poll` that closed it, or of the next `Poll` if it was closed outside one. `Stop` and `Dispose` close every connection without raising `ConnectionClosed`. `ConnectionReleased` fires on every path, including those, so it is the one place to drop a reference you kept.
...
| `InvalidOperationException: Cannot send on a connection that has been closed.` | A connection was kept and used after `ConnectionClosed`, `Stop` or `Dispose` | Drop the reference when `ConnectionClosed` fires, and when you call `Stop` or `Dispose` |
