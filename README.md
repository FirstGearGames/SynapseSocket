...
- **Sending on a closed `SynapseConnection` throws.** A closed connection is dead for good: connection objects are never reused, so a reference kept past close can never reach another session. `Send` on one throws `InvalidOperationException` and `Disconnect` on one does nothing. Its pooled buffers are released at the end of the `Poll` that closed it, or of the next `Poll` if it was closed outside one. `Stop` and `Dispose` close every connection without raising `ConnectionClosed`, so drop your references when you call them as well.
